...
- 最大前轮转角左右均 `20°`，固件默认 `APP_ORIN_ACKERMANN_MAX_STEERING_MRAD=349`。
- 舵机中位暂用 `1500 us`，两侧端点 `1105 us` / `1895 us`，固件默认 range `395 us`。
- 自动控制速度命令限幅：前进为 `10000 mm/s`（`10.00 m/s`），倒车仍为 `3000 mm/s`（`3.00 m/s`）；前向速度—PWM 表使用现有已配置最大值 `1650 us` 作为 `10.00 m/s` 初始端点，烧录后仍须依据霍尔反馈校准该段映射。
- 自动控制速度命令满足 `abs(speed_mmps) < 300` 时，速度目标回零且 ESC 输出中位；转向目标仍按命令执行。恰好 `300 mm/s` 可以进入速度映射。该阈值不用于篡改有效的 Hall 实际速度反馈。
- 霍尔反馈超速保护需要连续 3 次达到 `APP_HALL_SPEED_LIMIT_MMPS=12000` 才触发 ESC 回中，降至 `APP_HALL_SPEED_LIMIT_RELEASE_MMPS=10500` 后释放；单次霍尔速度跳变不会断开动力。这是正常前进命令上限之外的异常保护，不是自动控制命令限速。
- 固件速度目标变化限值为 `4.0 m/s²`，与当前 Simulator/Pilot 控制器和命令门控合同一致；它限制目标变化率，不代表实车实际加速度必然达到该值。
