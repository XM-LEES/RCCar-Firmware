...
| 制动映射中点 / F→R满请求端点 | µs | / | 中点须等于实际 ESC 中点；F→R刹车端点小于中点，独立于倒车推进限幅 |
| R→F满请求端点 | µs | | 大于中点，独立于前进推进限幅 |
...
| F→R / R→F刹车请求 | 0…1 | / | 两个方向分别标定，不预设相同 |
| F→R / R→F最短保持 | ms | / | 最终PWM连续达标的最短时间，分别标定 |
| F→R / R→F资格差值 | µs | / | 分别为`center-PWM`和`PWM-center`；使用最终实际PWM |
| 回中驻留 / 换向超时 | ms | / | 两个方向共用时序边界 |
...
新增默认项集中在 `app_vehicle_config.h` 的 `APP_ESC_MOTION_*`、`APP_ESC_TRACKING_BRAKE_*` 和 `APP_MODE2_*` 中。`APP_MODE2_FWD_TO_REV_*`与`APP_MODE2_REV_TO_FWD_*`必须分别按实车标定。代码使用零值与无效标志表示尚未标定；这不把零值定义为车辆物理参数。自动推进要求传动、停稳、模式二时序、双向制动映射、最终PWM资格差值和跟踪刹车配置全部有效。
