...
| RC方向确认样本 | 个不同FE32样本 | 2 | DRIVE、同一PWM侧且RPM高于停稳阈值；重复样本不累计 |
...
| F→R / R→F刹车请求 | 0…1 | 1.00 / 1.00 | 两个方向均要求完整行程，不用推测阈值 |
| F→R / R→F最短保持 | ms | 100 / 100 | 最终PWM连续达标的最短时间 |
| F→R / R→F资格差值 | µs | 500 / 500 | 分别为`center-PWM`和`PWM-center`；要求最终实际PWM到达完整端点 |
