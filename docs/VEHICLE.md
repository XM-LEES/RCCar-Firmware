...
| 刹车进入 / 释放误差 | m/s | 0.20 / 0.10 | 相对参考速度的误差滞回，独立于 12.0 / 10.5 m/s 绝对保护 |
...
| 回中驻留 / 换向超时 | ms | 100 / 5000 | 两个方向共用时序边界 |
...
新增默认项集中在 `app_vehicle_config.h` 的 `APP_ESC_MOTION_*`、`APP_ESC_TRACKING_BRAKE_*` 和 `APP_MODE2_*` 中。仓库默认值构成完整可运行配置，并保留`APP_MODE2_FWD_TO_REV_*`与`APP_MODE2_REV_TO_FWD_*`两个方向的独立调节能力。自动推进仍要求传动、停稳、模式二时序、双向制动映射、最终PWM资格差值和跟踪刹车配置全部有效。
