...
图定义组件与数据流，文字定义接口条件和控制规则。要逐步理解“怎么知道前进/倒车、RC和AUTO各自维护什么、速度为什么不能随便补零”，从[ESC观测与速度发布](ESC_OBSERVATION.md)开始阅读。
...
| 运动估计 | rpm_raw、低档轮轴系数、轮胎半径、方向观测结果 → 车速 | 幅值与方向分离；RC按FE32事件顺序确认方向，AUTO按独立模式二状态确认方向 |
...
软件复位或清除历史不代表电调已经复位。自动路径使用独立的模式二控制状态。人工RC输出保持直通，方向观测只读取软件PWM上下文和FE32反馈，不参与输出仲裁。RC的完整状态机、时序证据和失效规则见[ESC观测与速度发布](ESC_OBSERVATION.md)。
