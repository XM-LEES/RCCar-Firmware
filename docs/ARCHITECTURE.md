...
| RC 输入与输出仲裁 | RC、串口、停止覆盖 → 生效控制源 | PD12/PD13负责油门与转向接管；PD14作为未定用途AUX只采集，不参与仲裁 |
...
- 精确零目标表示停车；任意有限非零目标直接进入唯一的参考速度斜坡，得到`v参考`，不设软件速度死区或目标上限。
...
输出仲裁沿用基线：普通RC接管优先于串口源；无生效控制源时输出中点。PD14 AUX当前不具备停止语义。串口SOFTWARE_STOP是该命令源的停止覆盖，不是故障锁存；CLEAR_FAULT的受理条件与诊断语义见[接口约束](INTERFACES.md)。
