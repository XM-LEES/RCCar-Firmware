...
F→R资格判定以**最终实际输出PWM**相对中点的差值为准：1000 µs完整负向刹车连续保持至少100 ms，取得新停稳证据并完成回中驻留后，才允许再次负向推进。R→F使用2000 µs作为明确刹车请求，但只在FE32仍报告BRAKE时保持；BRAKE→DRIVE表示电调已经进入前进推进槽位，此时不能继续保持2000 µs。FE32动作未知时自动推进不授权。
