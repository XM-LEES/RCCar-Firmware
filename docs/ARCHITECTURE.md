...
| 倒车减速或转前进 | 当前自动路径仍按收油滑行和新停稳证据处理；实车RC已确认电调实际要求对称的刹车、停稳、回中、第二次正向输入，自动路径尚未对齐 |
...
软件复位或清除历史不代表电调已经复位。首次启动、RC 接管、反馈失效和换向超时之后，电调的倒车许可状态按未知处理；新停稳证据只能证明低速，不能证明下一次负向输入必定是刹车。第一版因此禁止未知状态下自动发出负向首击，也不会为了恢复状态自行向前驱动。人工 RC 输出仍保持直通；Hall 符号使用独立的对称模式二观察状态机，第一次相反输入期间维持原运动符号并等待停稳，停稳后回中、再次输入目标方向才换符号。
