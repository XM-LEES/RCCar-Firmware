...
| 启动或控制历史失效 | 状态记为UNKNOWN_SAFE；停稳时只允许受限的低PWM正向恢复探测，必须由探测开始后的新鲜运动样本确认，超时回中；禁止直接猜测倒车许可 |
| 换向超时或证据矛盾 | 进入禁止/不确定状态并输出中点，不自动尝试更强刹车 |

资格判定以**最终实际输出PWM**相对中点的差值为准，不以前级归一化请求替代。F→R与R→F分别配置刹车请求、最短保持时间和达标差值。普通弱刹不等于已满足换向许可；力度不足、中途被覆盖或先回中都会使本次资格失效。回中之后绝不在同一序列中发送“更强的刹车重试”，因为电调此时可能已将同侧信号解释为推进。夹入零速命令也不能绕过停稳与驻留要求。以上阈值与本车脉宽、时序的对应关系仍待实车标定。

软件复位或清除历史不代表电调已经复位。首次启动、RC 接管、反馈失效和换向超时之后，电调许可状态按未知处理；新停稳证据只能证明低速，不能证明下一次同侧输入究竟是刹车还是推进。自动路径禁止未知状态下直接发出负向动作。明确请求前进且已停稳时，只发送受限的正向恢复探测；软件继续保持方向未知，直到探测开始后的新鲜RPM样本超过停稳阈值，才进入FORWARD_TRACKING并允许有符号速度。探测超时则回中并保持未知。人工 RC 输出仍保持直通；Hall 符号使用独立的对称模式二观察状态机，第一次相反输入期间维持原运动符号并等待停稳，停稳后回中、再次输入目标方向才换符号。
...
ESC 反馈无效或关键观测、停稳、制动、模式二参数缺失时禁止自动闭环推进，不回退到 Hall 测速；恢复自动推进前重新取得停稳证据。当前所有新增实车阈值默认无效，因此能看到速度并不代表串口推进已经获准。24B状态位25–31直接报告授权、闭环、跟踪刹车、解锁、不确定、禁止和配置有效状态。转向与 RC 路径保留基线的输入有效性、限幅和停止处理。
