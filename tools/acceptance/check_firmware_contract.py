...
def check_vehicle_defaults(root: Path) -> list[Check]:
    ...
    expected = [
        ...
        "#define APP_ESC_TRACKING_BRAKE_ENTER_ERROR_MPS_DEFAULT 0.0f",
        "#define APP_ESC_TRACKING_BRAKE_RELEASE_ERROR_MPS_DEFAULT 0.0f",
    ...
    add(results, "vehicle_defaults", all(needle in text for needle in expected), "confirmed geometry, speed bounds, 0.3 m/s minimum, servo calibration, timeout, disabled unverified guard, and invalid-by-default ESC/mode2 config")
    add(
        results,
