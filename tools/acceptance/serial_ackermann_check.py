...
STATUS_ESC_SPEED_MAGNITUDE_VALID = 1 << 6
STATUS_ESC_STANDSTILL_CONFIRMED = 1 << 12
STATUS_HALL_SPEED_VALID = 1 << 13
STATUS_HALL_STOP_CONFIRMED = 1 << 19
STATUS_ESC_FE32_FRESH = 1 << 20
STATUS_ESC_RPM_RAW_VALID = 1 << 21
...
def parse_telemetry(frame: bytes) -> Telemetry:
    ...
    if not esc_magnitude_valid and speed_mps != 0.0:
        raise TelemetryContractError(
            "ESC speed is non-zero without speed-magnitude-valid status"
        )
    if speed_mps < 0.0 and not direction_known:
        raise TelemetryContractError(
            "negative ESC speed requires direction-known status"
        )
    if not direction_known and yaw_rate_radps != 0.0:
        raise TelemetryContractError(
            "yaw rate must be zero when vehicle direction is unknown"
        )

    hall_speed_mps = struct.unpack(">i", frame[3:7])[0] / 1000.0
    hall_valid = bool(status_bits & STATUS_HALL_SPEED_VALID)
    hall_standstill = bool(status_bits & STATUS_HALL_STOP_CONFIRMED)
    if hall_valid and hall_standstill:
        raise TelemetryContractError("Hall valid and stopped bits are both set")
    if not hall_valid and hall_speed_mps != 0.0:
...
def print_frames(frames: list[Telemetry], contract_errors: int) -> None:
    ...
    for frame in frames[-5:]:
        print(
            ...
            f"rpm_raw_valid={bool(frame.status_bits & STATUS_ESC_RPM_RAW_VALID)} "
            f"speed_cal={bool(frame.status_bits & STATUS_ESC_SPEED_CALIBRATION_VALID)} "
            f"dir_known={bool(frame.status_bits & STATUS_VEHICLE_DIRECTION_KNOWN)} "
            f"soft_uart_err={bool(frame.status_bits & STATUS_ESC_SOFT_UART_RX_ERROR)} "
