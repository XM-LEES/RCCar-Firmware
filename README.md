...
电调适配 MAX5 HV G2 模式二，处理前进、刹车和倒车。主车速由电调转速与传动模型换算。状态帧额外上传下位机按脉冲周期计算的 Hall 速度，供上位机独立对比；Hall 累计计数仍保留在固件内部。FE32 帧已通过电脑采集与原始数据回放；前进、后退分段直读进一步确认RPM只提供幅值，没有可靠方向位。C63A A3 链路实测持续 FE/NE，固件已改为从 PD15 以 EXTI + TIM5 定时采样接收 115200/8N1，并分层上报FE32新鲜度、raw RPM、标定、方向与接收错误。PD15 实板收帧、转速物理比例和闭环效果仍待验证。

新增传动、停稳和制动参数默认未标定，自动推进保持禁用。完整参数及有效标志确认前，不能把本版视作已完成实车标定的控制固件。
