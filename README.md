...
电调适配 MAX5 HV G2 模式二，处理前进、刹车和倒车。主车速幅值由实测低档传动系数先把`rpm_raw`换算到轮轴RPM，再结合独立轮胎半径计算。状态帧额外上传下位机按脉冲周期计算的 Hall 速度，供上位机独立对比；Hall 累计计数仍保留在固件内部。FE32 帧已通过电脑采集与原始数据回放；前进、后退分段直读确认RPM只提供幅值，没有可靠方向位。C63A A3 链路实测持续 FE/NE，PD15 EXTI + TIM5 软件UART已在实板观察到合法帧计数和raw RPM随油门变化；半位确认过滤出的伪起始位不再作为致命接收错误。闭环控制效果仍待验证。
